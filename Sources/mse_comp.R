...
mse_compare=function(enreg,rat){
  ...
  out <- DEoptim(sarsa_negLogLik, lower = c(0,0,0), upper = c(1,1,1), allpaths = allpaths_num, sim=2,DEoptim.control(NP = 20,F = 0.8, CR = 0.9, trace = FALSE, itermax = 200)) 
  alpha = out_sarsa$optim$bestmem[1]
  gamma = out_sarsa$optim$bestmem[2]
  lambda = out_sarsa$optim$bestmem[3]
  ...
  while(iter<=100){
    ...
    if(length(which(SMA(generated_data[,3],30)>=0.9))==0){
      next
    ...
    l<-which(SMA(generated_data[,3],30)>=0.90)
    k<-split(l, cumsum(c(1, diff(l) != 1)))
    for(set in 1:length(k)){
      if(length(k[[set]])>30){
        end_index=k[[set]][1]
    ...
    mse_model1 <- sum(colSums(mse_model1_mat[,(half_index+1):end_index]))/(end_index-half_index)
    ...
    mse_model2_mat <- computeMSE(generated_data,aca_model2_probMatrix,sim=1)
    mse_model2 <- sum(colSums(mse_model2_mat[,(half_index+1):end_index]))/(end_index-half_index)
    ...
    ## Compute MSE for Model2 using simulated data
    sarsa_probMatrix <-sarsaGetProbMatrix(generated_data, alpha_sarsa_est,gamma_sarsa_est,lambda_sarsa_est, matrix(0,nrow=2,ncol=6))
    mse_sarsa_mat <-computeMSE(allpaths_num,sarsa_probMatrix,sim=1)
    mse_sarsa <- sum(colSums(mse_sarsa_mat[,(half_index+1):end_index]))/(end_index-half_index)
    ...
    index_min=which.min(c(mse_model1,mse_model2,mse_sarsa))
    if(index_min==1){
