...
acaTurnData = function(generated_data, turnTimes, turnMethod, sim, half_index, end_index, window){
  # ACA = DEoptim(negLogLikFunc, lower = c(0,0), upper = c(1,0), allpaths = generated_data[1:half_index,], turnTimes = turnTimes, turnMethod = turnMethod, model=1, sim=sim, DEoptim.control(NP=100, F=2, CR = 0.9,trace = FALSE, itermax = 200, strategy = 1))
  # alpha_ACA = ACA$optim$bestmem[1]
  # reward_ACA = ACA$optim$bestmem[2]
  # reward_ACA = 1+reward_ACA*9
  
  ACA = ga("real-valued", fitness =  negLogLikFunc,lower = c(0), upper = c(1),allpaths = generated_data[1:half_index,],  turnTimes = turnTimes, turnMethod = turnMethod, model = 1, sim = sim, popSize=100, maxiter = 1000, run = 50,monitor=FALSE) 
...
gbTurnData = function(generated_data, turnTimes, turnMethod, sim, half_index, end_index, window){
  # GB = DEoptim(negLogLikFunc, lower = c(0,0), upper = c(1,0), allpaths = generated_data[1:half_index,], turnTimes = turnTimes, turnMethod = turnMethod, model=2, sim=sim, DEoptim.control(NP=100, F=2, CR = 0.9,trace = FALSE, itermax = 200, strategy = 1))
  # alpha_GB = GB$optim$bestmem[1]
  # reward_GB = GB$optim$bestmem[2]
  # reward_GB = 1+reward_GB*9
  GB = ga("real-valued", fitness =  negLogLikFunc,lower = c(0), upper = c(1),allpaths = generated_data[1:half_index,],  turnTimes = turnTimes, turnMethod = turnMethod, model = 2, sim = sim, popSize=100, maxiter = 1000, run = 50,monitor=FALSE)
...
aca2TurnData = function(generated_data, turnTimes, turnMethod, sim, half_index, end_index, window){
  # ACA2 <- DEoptim(negLogLikFunc,lower = c(0,0), upper = c(1,1), allpaths = generated_data[1:half_index,],  turnTimes = turnTimes, turnMethod = turnMethod, model = 4, sim = sim, DEoptim.control(NP=100, F=2, CR = 0.9,trace = FALSE, itermax = 200, strategy = 1))
  ...
  ACA2 = ga("real-valued", fitness =  negLogLikFunc,lower = c(0,0), upper = c(1,1),allpaths = generated_data[1:half_index,],  turnTimes = turnTimes, turnMethod = turnMethod, model = 4, sim = sim, popSize=100, maxiter = 1000, run = 50,monitor=FALSE)
...
aca3TurnData = function(generated_data, turnTimes, turnMethod, sim, half_index, end_index, window){
  # ACA3 <- DEoptim(negLogLikFunc,lower = c(0,0,0,0), upper = c(1,1,1,0), allpaths = generated_data[1:half_index,],  turnTimes = turnTimes, turnMethod = turnMethod, model = 5, sim = sim, DEoptim.control(NP=100, F=2, CR = 0.9,trace = FALSE, itermax = 400,strategy = 2))
  # alpha_ACA3 = ACA3$optim$bestmem[1]
  ...
  ACA3 = ga("real-valued", fitness =  negLogLikFunc,lower = c(0,0,0), upper = c(1,1,1),allpaths = generated_data[1:half_index,],  turnTimes = turnTimes, turnMethod = turnMethod, model = 5, sim = sim, suggestions = suggestedSol, popSize=500, maxiter = 1000, run = 50,monitor=FALSE, optim = TRUE)
...
sarsaTurnData=function(generated_data,turnTimes, sim, half_index, end_index, window){
  # SARSA <- DEoptim(negLogLikFunc,lower = c(0,0,0,0), upper = c(1,1,1,1), allpaths = generated_data[1:half_index,], turnTimes = 0, turnMethod = 0, model = 6, sim = sim, DEoptim.control(NP=100, F=2, CR = 0.9,trace = FALSE, itermax = 200, strategy = 1))
  ...
  out <- GenSA(lower = c(0,0,0,0), upper = c(1,1,1,1), fn = negLogLikFunc,allpaths = generated_data[1:half_index,],  turnTimes = turnTimes, turnMethod = turnMethod, model = 6, sim = sim,control = list(verbose=FALSE,max.time=120))
  alpha_SARSA = out$par[1]
  gamma_SARSA = out$par[2]
  lambda_SARSA = out$par[3]
  reward_SARSA = out$par[4]
  reward_SARSA = 1 + reward_SARSA*9
...
negLogLikFunc=function(par,allpaths,turnTimes,turnMethod,model,sim) {
  ...
  if(model == 1 || model == 2 || model == 3){
    ...
    if(path4Converged &&  path10Converged)
    {
      turnlik=Aca3Turns::getTurnsLikelihood(allpaths,turnTimes,turnMethod,alpha,gamma1,gamma2,1,sim)
    }
    else
    {
      turnlik = -1000000
