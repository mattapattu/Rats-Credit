...
// [[Rcpp::export()]]
arma::vec getTurnsLikelihood(arma::mat allpaths, arma::mat boxTimes, double alpha, arma::vec H, int sim, int model)
{
  ...
  for (unsigned int session = 0; session < (uniqSessIdx.n_elem - 1); session++)
  {
    ...
    arma::uvec boxTimes_idx = arma::find(boxTimes.col(4) == sessId);
    arma::vec turn_times_session = allpath_times.elem(boxTimes_idx);
    ...
    std::vector<std::shared_ptr<TreeNode>> episodeTurnTimes;
